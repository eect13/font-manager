...
// 1.0.206h: repo cwd has src/lib/og/site.json + public/og.jpg — without an isolated
// cwd, injectGrokPwaHead/normalizeHeadContext stamps Font Manager identity into unit
// tests that expect platform defaults / placeholder cards. Root cause = cwd pollution.
const ISOLATED_CWD = mkdtempSync(join(tmpdir(), "grok-og-isolated-"));
function inject(html, ctx = {}) {
  return injectGrokPwaHead(html, { ...ctx, cwd: ctx.cwd ?? ISOLATED_CWD });
}
function makeInjector(ctx = {}) {
  return createHeadInjector({ ...ctx, cwd: ctx.cwd ?? ISOLATED_CWD });
}
...
test("cwd public/og.jpg stamps card=custom onto an explicit site without card", () => {
  // Production intentionally applies disk og.jpg over placeholder (bake/watch).
...
test("isolated cwd + site without card keeps og.grok.me placeholder", () => {
  const out = inject("<html><head></head></html>", {
    host: "wild-race.grok.me",
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = makeInjector({ appName: "Wild Race" });
  const chunks = [
