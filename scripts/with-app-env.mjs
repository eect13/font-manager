 ...
 *
 * On Windows, npm's `vite` shim is `node_modules/.bin/vite.cmd`. Node's
 * `spawn("vite")` without a shell looks for `vite.exe` and throws ENOENT.
 * Local `.bin` is prepended to PATH, and cmd.exe is used for non-absolute
 * commands so those shims resolve.
 */
...
/** Put `node_modules/.bin` first so `vite` resolves without relying on npm's PATH. */
export function withLocalBinPath(env, root = projectRoot()) {
  const binDir = localBinDir(root);
  const next = { ...env };
  const pathKey =
    process.platform === "win32"
      ? Object.keys(next).find((k) => k.toLowerCase() === "path") || "Path"
      : "PATH";
  const current = next[pathKey] ?? "";
  const parts = current.split(delimiter).filter(Boolean);
  if (!parts.includes(binDir)) {
    next[pathKey] = parts.length ? `${binDir}${delimiter}${current}` : binDir;
  }
  return next;
}

/** Prefer the local shim (`vite.cmd` on Windows) when the command is a bare name. */
export function resolveCommand(command, root = projectRoot(), platform = process.platform) {
  if (!command || isAbsolute(command) || /[\\/]/.test(command)) return command;
  const binDir = localBinDir(root);
  // On Windows, keep the bare name (`vite`) so cmd.exe can pick `vite.cmd`
  // via PATH + shell. Passing the `.cmd` path without a shell is ENOENT.
  if (platform === "win32") return command;
  const posix = join(binDir, command);
  return existsSync(posix) ? posix : command;
}
