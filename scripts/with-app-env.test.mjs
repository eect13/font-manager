...
test("local node_modules/.bin is prepended to PATH", () => {
  ...
  const env = withLocalBinPath({ PATH: "/usr/bin" }, root);
  assert.equal(env.PATH.startsWith(`${bin}:`) || env.PATH.startsWith(`${bin};`), true);
});

test("Windows .cmd shims use a shell; absolute binaries do not", () => {
...
test("bare vite resolves to the local binary on POSIX", () => {
  const resolved = resolveCommand("vite", projectRoot(), "linux");
  assert.equal(resolved.includes(join("node_modules", ".bin")), true);
  assert.equal(resolveCommand("vite", projectRoot(), "win32"), "vite");
});

test("the wrapper finds local node_modules binaries (vite --version)", async () => {
  const { stdout, stderr } = await execFileAsync(process.execPath, [WRAPPER, "vite", "--version"]);
  const out = `${stdout}${stderr}`;
  assert.match(out, /vite\/\d/i);
});
