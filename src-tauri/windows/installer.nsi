...
Function PageLeaveReinstall
  ...
  reinst_uninstall:
    ...
    ${Else}
      ...
      ClearErrors
      ; Quote paths — product name contains a space.
      ExecWait '"$4\uninstall.exe" /S _?=$4' $0
    ${EndIf}
    ...
    ${OrIf} ${FileExists} "$4\${PRODUCTNAME}.exe"
      ...
      ${AndIf} $0 = 1602
        Abort
      ${EndIf}
      ; NSIS $0=1 is "Unable to uninstall", not user cancel. Overwrite.
      Goto force_clean_old
    ${EndIf}
    Goto reinst_done
    ...
    force_clean_old:
      BringToFront
      Call KillFontManager
      ${StrLoc} $R8 `$4` `${PRODUCTNAME}` `>`
      ${If} $4 != ""
      ${AndIf} $R8 != ""
        RMDir /r `$4`
      ${EndIf}
      ${StrLoc} $R8 `$INSTDIR` `${PRODUCTNAME}` `>`
      ${If} $INSTDIR != ""
      ${AndIf} $R8 != ""
        RMDir /r `$INSTDIR`
      ${EndIf}
