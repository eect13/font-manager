...
- **Amend (Skye 8.5 No-lie):** post-end min-display hold (`holdDocsCancelChrome` / job ended, dismiss path) must not paint Cancel Documents refresh while `onClick` is only `dismissDownloadBar`. Honest **Dismiss** via `cancelChromeA11y({ dismissHold: true })` — `aria-label="Dismiss Documents refresh"`, `fm-dismiss-documents-refresh`, `data-cancel-kind="dismiss"`, visible **Dismiss**; label **Refreshing complete**. Bar may stay for remaining min window; honesty > fake Cancel.
- **Min display (docs path only):** if docs refresh would clear the bar before 1s, hold chrome until `paintedAt+1000`; hold → Dismiss / `dismissDownloadBar` only (no Pause / no fake cancellable job). Activate/Restore paths unchanged.
