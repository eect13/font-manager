...
**1.0.206w (Eric GO via Skye — Documents refresh cancel toast honesty).** Cancel mid-Refresh Documents is owned by `cancelDownloadQueue`: sticky `docsVfSyncCancelPending` / `wasDocsVfSync` (+ job.current belt-and-suspenders) toasts **Documents refresh cancelled** before any await and never emits **Download cancelled**. Callers skip double-toast via `didDocsVfSyncCancelToast`. Progress-bar Cancel keeps `data-testid="activate-bar-cancel"` with stable aria-label **Cancel Documents refresh** (+ `fm-cancel-documents-refresh`) during docs sync; bar label prefers Refreshing Documents over bare Downloading. Optional: session GDI restore Cancel → **Session restore cancelled**. 206v ACL / 206u VF-primary kept. ProductVersion stays **1.0.206**. No tip-install/pack this tip.
