...
**X** quits the app (session fonts unload; Documents files stay). A family is **ready** only with a `.complete` marker stamped when the on-disk face count matches the expected full set — partial downloads do not stamp, and Scan/hydrate deletes lying `.complete` markers (including legacy bare `"1"` with no expected face count, and official Google Fontsource latin packs stamped complete without `.google-planned` or below the catalog weights×italic floor) so Repair appears. Ready families register again on next launch — no re-download.

**Google install path.** For catalog-**variable** families, Activate prefers Google CSS axis-range requests (`ital,wght@0,min..max;1,min..max`) that yield installable desktop TTFs via Mozilla/Googlebot (instance TTFs covering the range) — never Chrome/Safari WOFF2 and never `@fontsource-variable` WOFF. 400-swept range listings are rejected; static `ital,wght@0|1,w` remains the fallback (bare `family=` Regular-400 cannot stamp `.complete` when a richer listing exists). Face downloads stream to disk with a global in-flight cap and unbounded cancel-aware slot waits (no ~30s silent drop under bulk workers). `.complete` is gated on Google face keys (`{slug}-{w}-{s}.ttf`) and requires `.google-planned` — Fontsource `*-latin-*` extras never pad the planned count and are purged on Google re-download so leftovers cannot clash. Non-Google (`catalog: other`) families skip Google CSS and use Fontsource only. Fontsource remains the catalog listing for exclusive `type: other` families and the fill path when Google listed **0** faces (`need_fontsource = google_expected == 0`) — partial Google never falls back to Fontsource (subset names cannot satisfy Google face keys). On upgrade to 1.0.147 a one-shot Documents walk applies Google-complete honesty (old verify-based clear would find 0 latin lies).
