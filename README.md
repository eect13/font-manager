...
**1.0.206u (Eric GO — VF-primary + Refresh Documents).** Catalog-variable families with an intact VF treat the variable face as primary: Activate Adds only `*-variable-*` (~2 for Inter, not ~20 statics); download/planned = vars only (no static backup); redundant statics purged from Documents + gdi-maps (safe delete; locked → soft toast). Poppins / no-public-VF denylist / static-only stay all-statics. Manual **Refresh Documents** (Library ⋯ + Settings) reconciles installed folders to VF policy with progress/Cancel — no surprise full-catalog download. Activate remaining aria-label matches visible text. 206t #185 / useShallow / TIP_SHA kept. ProductVersion stays **1.0.206**. Pack HOLD. No tip-install/pack this tip.
