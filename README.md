...
**Google install path.** For catalog-**variable** families, Activate still pulls Google CSS instance TTFs (Mozilla/Googlebot) for Adobe-friendly static faces, and **also** downloads the real variable TTFs from `google/fonts` via jsDelivr (`Nunito[wght].ttf` → `nunito-variable-wght.ttf`). When axis-range CSS only yields range-weight keys (`font-weight: 200 1000` → `200-1000`), planned keys prefer the discrete static `ital,wght@0|1,w` listing instead. Google CSS instances are name-patched so Illustrator sees family `Nunito` + style `ExtraLight` (not family `Nunito ExtraLight`). Never Chrome/Safari WOFF2 and never `@fontsource-variable` WOFF. `.complete` / `.google-planned` count instances **plus** variable files. Latin remnants are not purged into an empty folder when a Google fetch writes nothing. Fontsource fills only when Google listed **0** faces.
